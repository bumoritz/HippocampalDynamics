function F = singleCellFigure(info,p,tng,idx,paq_beh,suffix,limits,trialTypes)

if nargin<7
    limits = [];
end
if nargin<8
    trialTypes = 'AB_AY_XY_XB';
end
...
if strcmp(trialTypes,'AB_AY_XY_XB')
    if compact_version
        F = default_figure([-20,0.5,3.5,5]);
        nrows = 8;
        ncols = 1;
    else
...
elseif (strcmp(trialTypes,'A_X') || strcmp(trialTypes,'X_A'))
    F = paper_figure([0,0.5,mm2inch(0.9*34),mm2inch(1.2*34)]); hold on;
end
...
origAvgData_A = permute(squeeze(tng.avgTraces.(['A',suffix])(idx,:)),[2,1]);
origAvgData_X = permute(squeeze(tng.avgTraces.(['X',suffix])(idx,:)),[2,1]);
origAvgData_B = permute(squeeze(tng.avgTraces.(['B',suffix])(idx,:)),[2,1]);
origAvgData_Y = permute(squeeze(tng.avgTraces.(['Y',suffix])(idx,:)),[2,1]);
temp = [origAvgData_A;origAvgData_X;origAvgData_B;origAvgData_Y];
origAvgData_lower = nanmin(temp(:));
origAvgData_upper = nanmax(temp(:));

% extract data
if (strcmp(trialTypes,'A_X') || strcmp(trialTypes,'X_A'))
    data_A = permute(squeeze(tng.traces.A_var0(idx,:,:)),[2,1]);
    data_X = permute(squeeze(tng.traces.X_var0(idx,:,:)),[2,1]);
end
data_AB = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['stimuli_inA',suffix]).AB)),[2,1]);
data_AY = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['stimuli_inA',suffix]).AY)),[2,1]);
data_XY = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['stimuli_inX',suffix]).XY)),[2,1]);
data_XB = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['stimuli_inX',suffix]).XB)),[2,1]);
% data_AB_H = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['outcome_inA',suffix]).A_H)),[2,1]);
% data_AY_CR = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['outcome_inA',suffix]).A_CR)),[2,1]);
% data_XY_H = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['outcome_inX',suffix]).X_H)),[2,1]);
% data_XB_CR = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['outcome_inX',suffix]).X_CR)),[2,1]);
% data_AB_M = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['outcome_inA',suffix]).A_M)),[2,1]);
% data_AY_FA = permute(squeeze(tng.traces.(['A',suffix])(idx,:,tng.trials.(['outcome_inA',suffix]).A_FA)),[2,1]);
% data_XY_M = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['outcome_inX',suffix]).X_M)),[2,1]);
% data_XB_FA = permute(squeeze(tng.traces.(['X',suffix])(idx,:,tng.trials.(['outcome_inX',suffix]).X_FA)),[2,1]);
...
% average data
if (strcmp(trialTypes,'A_X') || strcmp(trialTypes,'X_A'))
    avgData_A = nanmean(data_A,1);
    avgData_X = nanmean(data_X,1);
end
...
% normalise data (used for paper example imprinting traces)
if (strcmp(trialTypes,'A_X') || strcmp(trialTypes,'X_A'))
    temp = [avgData_A;avgData_X];
    this_lower = nanmin(temp(:))
    this_upper = nanmax(temp(:))
    normData_traces_A = (data_A-this_lower) ./ (this_upper-this_lower);
    normData_traces_X = (data_X-this_lower) ./ (this_upper-this_lower);
end
...
if strcmp(trialTypes,'AB_AY_XY_XB')
    ...
    plt = struct(); plt.ylim = [-0.2,1.2]; %[avgNormData_exclSplit_lower-0.2,avgNormData_exclSplit_upper+0.2]; 
    traces_task(tng,p,info,plt); 
    hold off
end
...
if (strcmp(trialTypes,'A_X') || strcmp(trialTypes,'X_A'))
    subplot(2,1,1)
    hold on
    plt = struct(); plt.ylim = [-0.2,1.4]; % [-0.2,1.2]
    this_t = p.general.t_binned;
    %ymin = -0.2; ymax = 1.4;
    ymin = -0.1; ymax = 1.2;
    if strcmp(trialTypes,'A_X')
        this_start = p.general.t_binned(tng.firingField.Astim_AW.boundaries(idx,1));
        this_stop = p.general.t_binned(tng.firingField.Astim_AW.boundaries(idx,2));
        patch([interp1(this_t,1:length(this_t),this_start),interp1(this_t,1:length(this_t),this_start),interp1(this_t,1:length(this_t),this_stop),interp1(this_t,1:length(this_t),this_stop)],...
            [ymin,ymax,ymax,ymin],mean([p.col.photostim;p.col.white;p.col.white]),'EdgeColor','none');  
        traces_task(tng,p,info,plt); %traces_task(tng,p,info,plt,1); 
        temp=shadedErrorBar(1:size(normData_traces_X,2),nanmean(normData_traces_X,1),nansem(normData_traces_X,1),'lineProps',p.col.darkGray); temp.mainLine.LineWidth = 1;   
        temp=shadedErrorBar(1:size(normData_traces_A,2),nanmean(normData_traces_A,1),nansem(normData_traces_A,1),'lineProps',p.col.seq); temp.mainLine.LineWidth = 1;   
    elseif strcmp(trialTypes,'X_A')
        this_start = p.general.t_binned(tng.firingField.Xstim_AW.boundaries(idx,1));
        this_stop = p.general.t_binned(tng.firingField.Xstim_AW.boundaries(idx,2));
        patch([interp1(this_t,1:length(this_t),this_start),interp1(this_t,1:length(this_t),this_start),interp1(this_t,1:length(this_t),this_stop),interp1(this_t,1:length(this_t),this_stop)],...
            [ymin,ymax,ymax,ymin],mean([p.col.photostim;p.col.white;p.col.white]),'EdgeColor','none');
        traces_task(tng,p,info,plt); %traces_task(tng,p,info,plt,1); 
        temp=shadedErrorBar(1:size(normData_traces_A,2),nanmean(normData_traces_A,1),nansem(normData_traces_A,1),'lineProps',p.col.darkGray); temp.mainLine.LineWidth = 1;   
        temp=shadedErrorBar(1:size(normData_traces_X,2),nanmean(normData_traces_X,1),nansem(normData_traces_X,1),'lineProps',p.col.seq); temp.mainLine.LineWidth = 1;   
    end
    hold off
end
...
if ~compact_version
    ...
    plt.color = p.col.darkGray;
    myHistogram(tng.firingField.X_AW.selectivity_ipsi(tng.passed.AW.X==1),p,plt); temp2 = get(gca,'ylim');
    hold off

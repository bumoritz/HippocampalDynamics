function [F,these_normAvgTraces,these_idcs_sorted,plt] = snakePlots(this_tracesStruct,these_idcs_unsorted,these_idcs_sorted,these_refs_sort,these_refs_norm,p,info,plt)
...
if numBlocks==1
    ...
    for j=1:numBlocks
        ...
        elseif numSequences~=1
            if temp=="A" && strcmp(plt.split,'none')
                these_avgTraces{1,j} = nanmean(this_tracesStruct{j}.A,3);
                numTrials = [numTrials,size(this_tracesStruct{j}.A,3)];
            elseif temp=="X" && strcmp(plt.split,'none')
                these_avgTraces{1,j} = nanmean(this_tracesStruct{j}.X,3);
                numTrials = [numTrials,size(this_tracesStruct{j}.X,3)];
            end
        end
    end
...
if isempty(these_idcs_sorted)
    ...
    for i=1:length(these_refs_sort)
        if ~isnan(these_refs_sort(i))
            if numBlocks > 1 && numSequences > 1
                [~,temp] = nanmax(these_avgTraces{1,these_refs_sort(i)}(these_idcs_unsorted{i},this_sortingWindow),[],2);
                [~,temp2] = sort(temp);
    ...
    for r=1:nrows
        ...
        for c=1:ncols
            ...
            if strcmp(plt.normalisationType,'one-sided')
                if isfield(plt,'lower')
                    %this_lower = plt.lower{r};
                    this_lower = plt.lower;
                else
...
for r=1:nrows
    for c=1:ncols
        ...
        if numBlocks > 1 && numSequences > 1
            heatMap_task(these_normAvgTraces{r,c}(these_idcs_sorted{c},:),NaN,[],p,info,plt);
        else
...
% % rate map correlations (AW) between odd and even od he same trial type
% 
% % A - odd vs even:  
% these_data_1 = these_normAvgTraces{1,1}(these_idcs_sorted{1},p.general.bins_analysisWindow);
% these_data_2 = these_normAvgTraces{1,2}(these_idcs_sorted{1},p.general.bins_analysisWindow);
% these_corrs = nan(size(these_data_1,1),1);
% for i=1:size(these_data_1,1)
%     these_corrs(i) = corr(these_data_1(i,:)',these_data_2(i,:)','Type','Pearson','Rows','Complete');
% end
% this_avgCorr = nanmean(these_corrs)
% 
% % X - odd vs even:  
% these_data_1 = these_normAvgTraces{2,3}(these_idcs_sorted{2},p.general.bins_analysisWindow);
% these_data_2 = these_normAvgTraces{2,4}(these_idcs_sorted{2},p.general.bins_analysisWindow);
% these_corrs = nan(size(these_data_1,1),1);
% for i=1:size(these_data_1,1)
%     these_corrs(i) = corr(these_data_1(i,:)',these_data_2(i,:)','Type','Pearson','Rows','Complete');
% end
...
% % A vs X - odd - Aseq:  
% these_data_1 = these_normAvgTraces{1,1}(these_idcs_sorted{1},p.general.bins_analysisWindow);
% these_data_2 = these_normAvgTraces{1,3}(these_idcs_sorted{1},p.general.bins_analysisWindow);
% these_corrs = nan(size(these_data_1,1),1);
% for i=1:size(these_data_1,1)
%     these_corrs(i) = corr(these_data_1(i,:)',these_data_2(i,:)','Type','Pearson','Rows','Complete');
% end
...
% 
% % A vs X - odd - Xseq:  
% these_data_1 = these_normAvgTraces{2,1}(these_idcs_sorted{2},p.general.bins_analysisWindow);
% these_data_2 = these_normAvgTraces{2,3}(these_idcs_sorted{2},p.general.bins_analysisWindow);
% these_corrs = nan(size(these_data_1,1),1);
% for i=1:size(these_data_1,1)
%     these_corrs(i) = corr(these_data_1(i,:)',these_data_2(i,:)','Type','Pearson','Rows','Complete');
% end

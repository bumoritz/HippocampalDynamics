function [tngnn,nft_binned] = tuningAnalysisDffS(info,iscell,ops,p,path,task,s2p_meta,sync_beh,act)
...
[prop,nf_binned,nft_binned] = preprocessActivityMeasure(act,p.tngnn,p,sync_beh,iscell);
...
if ops.tngnn.do_60t || ops.tngnn.do_60t_onlyFirst
...
if ops.tngnn.do_100t || ops.tngnn.do_100t_onlyFirst
    trials_100t = {}; traces_100t = {}; avgTraces_100t = {}; normAvgTraces_100t = {};
    if ~ops.tngnn.do_100t
        temp = 1;
    else
...
% stimVersion
if ops.tngnn.do_allTrials_stimVersion
...
if ops.tngnn.do_100t_stimVersion
...
if ops.tngnn.do_allTrials
    if p.tngnn.analysisBlockRestrictedNf
    ...
    if ops.tngnn.do_eventWiseAnalysis
...
% end
if strcmp(p.tngnn.activityMeasure,"casc_50c_beh")
    this_activity_measure = 'casc_50c';
elseif strcmp(p.tngnn.activityMeasure,"casc_50g_beh")
    this_activity_measure = 'casc_50g';
elseif strcmp(p.tngnn.activityMeasure,"casc_100c_beh")
    this_activity_measure = 'casc_100c';
elseif strcmp(p.tngnn.activityMeasure,"casc_100g_beh")
    this_activity_measure = 'casc_100g';
elseif strcmp(p.tngnn.activityMeasure,"casc_200g_beh")
    this_activity_measure = 'casc_200g';
elseif strcmp(p.tngnn.activityMeasure,"spks_beh")
    this_activity_measure = 'spks';
elseif strcmp(p.tngnn.activityMeasure,"dFF_beh")
    this_activity_measure = 'dFF';
elseif strcmp(p.tngnn.activityMeasure,"spksn_beh")
    this_activity_measure = 'spksn';
elseif strcmp(p.tngnn.activityMeasure,"spksnn_beh")
    this_activity_measure = 'spksnn';
elseif strcmp(p.tngnn.activityMeasure,"dFFn_beh")
    this_activity_measure = 'dFFn';
elseif strcmp(p.tngnn.activityMeasure,"dFFnn_beh")
    this_activity_measure = 'dFFnn';
elseif strcmp(p.tngnn.activityMeasure,"dFFns_beh")
    this_activity_measure = 'dFFns';
elseif strcmp(p.tngnn.activityMeasure,"F_beh")
...
%         this_activity_measure,'-smo',num2str(p.tngnn.smoothingSd_preBinning),'-bin',num2str(p.general.binSize),', ',...
...
%     saveas(F,[path.filepart_outX,'plots/SnakePlots/',info.animal,'_',info.date,'_',this_activity_measure,'_','snakePlot_AXonly_allTrials_8tile_twosided_oddeven_13.png']);
    ...
    if ~ops.tngnn.skip_boringSnakePlots
...
if ops.tngnn.do_60t && ops.tngnn.do_allTrials
...
if ops.tngnn.do_allTrials
    tngnn_all.shuffling = shuffling_all;
    tngnn_all.firingField = firingField_all;
    tngnn_all.passed = passed_all;
    tngnn_all.passed_stats = passed_stats_all;
    tngnn_all.prop = prop;
    tngnn_all.p = p;
    tngnn_all = orderfields(tngnn_all);
    save([path.filepart_out,'tngnn_all.mat'],'tngnn_all','-v7.3');
    tngnn_all_cmpr.passed = passed_all.AW;
    tngnn_all_cmpr.passed_stats = passed_stats_all.AW;
    save([path.filepart_out,'tngnn_all_cmpr.mat'],'tngnn_all_cmpr','-v7.3');
    disp(['--- Saved tngnn_all file as ',[path.filepart_out,'tngnn_all.mat'],'.'])
...
if ops.tngnn.do_60t || ops.tngnn.do_60t_onlyFirst
    ...
    for i=1:temp
%         tngnn_60t{i}.trials = trials_60t{i};
%         tngnn_60t{i}.traces = traces_60t{i};
%         tngnn_60t{i}.avgTraces = avgTraces_60t{i};
%         tngnn_60t{i}.normAvgTraces = normAvgTraces_60t{i};
        tngnn_60t{i}.shuffling = shuffling_60t{i};
        tngnn_60t{i}.firingField = firingField_60t{i};
        tngnn_60t{i}.passed = passed_60t{i};
        tngnn_60t{i}.passed_stats = passed_stats_60t{i};
%         tngnn_60t{i}.passed_nneg = passed_nneg_60t{i};
%         tngnn_60t{i}.passed_stats_nneg = passed_stats_nneg_60t{i};
        tngnn_60t{i}.prop = prop;
        tngnn_60t{i}.p = p;
        tngnn_60t{i} = orderfields(tngnn_60t{i});
    end
    save([path.filepart_outX,'tngnn_60t.mat'],'tngnn_60t','-v7.3');
    disp(['--- Saved tngnn_60t file as ',[path.filepart_outX,'tngnn_60t.mat'],'.'])
end
if ops.tngnn.do_100t || ops.tngnn.do_100t_onlyFirst
    tngnn_100t = {};
    if ~ops.tngnn.do_100t
    ...
    for i=1:temp
%         tngnn_100t{i}.trials = trials_100t{i};
%         tngnn_100t{i}.traces = traces_100t{i};
%         tngnn_100t{i}.avgTraces = avgTraces_100t{i};
%         tngnn_100t{i}.normAvgTraces = normAvgTraces_100t{i};
        tngnn_100t{i}.shuffling = shuffling_100t{i};
        tngnn_100t{i}.firingField = firingField_100t{i};
        tngnn_100t{i}.passed = passed_100t{i};
        tngnn_100t{i}.passed_stats = passed_stats_100t{i};
%         tngnn_100t{i}.passed_nneg = passed_nneg_100t{i};
%         tngnn_100t{i}.passed_stats_nneg = passed_stats_nneg_100t{i};
        tngnn_100t{i}.prop = prop;
        tngnn_100t{i}.p = p;
        tngnn_100t{i} = orderfields(tngnn_100t{i});
        tngnn_100t_cmpr{i}.passed = passed_100t{i}.AW;
        tngnn_100t_cmpr{i}.passed_stats = passed_stats_100t{i}.AW;
%         tngnn_100t_cmpr{i}.passed_nneg = passed_nneg_100t{i}.AW;
%         tngnn_100t_cmpr{i}.passed_stats_nneg = passed_stats_nneg_100t{i}.AW;
    end
    save([path.filepart_outX,'tngnn_100t.mat'],'tngnn_100t','-v7.3');
    save([path.filepart_out,'tngnn_100t_cmpr.mat'],'tngnn_100t_cmpr','-v7.3');
    disp(['--- Saved tngnn_100t file as ',[path.filepart_outX,'tngnn_100t.mat'],'.'])
end
if ops.tngnn.do_allTrials
    tngnn = tngnn_all;
elseif ops.tngnn.do_60t || ops.tngnn.do_60t_onlyFirst
    tngnn = tngnn_60t{1};
elseif ops.tngnn.do_100t || ops.tngnn.do_100t_onlyFirst
    tngnn = tngnn_100t{1};
end

% stimVersion
if ops.tngnn.do_allTrials_stimVersion
    tngnn_all_stimVersion.shuffling = shuffling_all_stimVersion;
    tngnn_all_stimVersion.firingField = firingField_all_stimVersion;
    tngnn_all_stimVersion.passed_catch = passed_all_stimVersion_catch;
    tngnn_all_stimVersion.passed_stats_catch = passed_stats_all_stimVersion_catch;
    tngnn_all_stimVersion.passed_stim = passed_all_stimVersion_stim;
    tngnn_all_stimVersion.passed_stats_stim = passed_stats_all_stimVersion_stim;
    tngnn_all_stimVersion.prop = prop;
    tngnn_all_stimVersion.p = p;
    tngnn_all_stimVersion = orderfields(tngnn_all_stimVersion);
    save([path.filepart_outX,'tngnn_all_stimVersion.mat'],'tngnn_all_stimVersion','-v7.3');
    disp(['--- Saved tngnn_all_stimVersion file as ',[path.filepart_outX,'tngnn_all_stimVersion.mat'],'.'])    
end
if ops.tngnn.do_100t_stimVersion
    tngnn_100t_stimVersion = {};
    for i=1:floor(prop.numTrials/100)
        tngnn_100t_stimVersion{i}.shuffling = shuffling_100t_stimVersion{i};
        tngnn_100t_stimVersion{i}.firingField = firingField_100t_stimVersion{i};
        tngnn_100t_stimVersion{i}.passed_catch = passed_100t_stimVersion_catch{i};
        tngnn_100t_stimVersion{i}.passed_stats_catch = passed_stats_100t_stimVersion_catch{i};
        tngnn_100t_stimVersion{i}.passed_stim = passed_100t_stimVersion_stim{i};
        tngnn_100t_stimVersion{i}.passed_stats_stim = passed_stats_100t_stimVersion_stim{i};
        tngnn_100t_stimVersion{i}.prop = prop;
        tngnn_100t_stimVersion{i}.p = p;
        tngnn_100t_stimVersion{i} = orderfields(tngnn_100t_stimVersion{i});
    end
    save([path.filepart_outX,'tngnn_100t_stimVersion.mat'],'tngnn_100t_stimVersion','-v7.3');
    disp(['--- Saved tngnn_100t_stimVersion file as ',[path.filepart_outX,'tngnn_100t_stimVersion.mat'],'.'])
end
if ops.tngnn.do_allTrials_stimVersion
    tngnn_stimVersion = tngnn_all_stimVersion;
elseif ops.tngnn.do_100t_stimVersion
    tngnn_stimVersion = tngnn_100t_stimVersion{1};
end

% try
%     idx = 1;
%     population = find(tngnn.passed.AW.A==1);
%     F = singleCellFigure(info,p,tngnn,population(idx),sync_beh);

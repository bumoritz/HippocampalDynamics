...
if ~exist([path.filepart_outX,'plots'],'dir')
    mkdir([path.filepart_outX,'plots']);
end
...
if ops.do_imprintingAnalysis
	cmpr_list = [cmpr_list,meta.reg.trg,"task","s2p_meta","paq_beh",p.impr.activityMeasure];
end
...
if ops.do_saveTraces
    cmpr_list = [cmpr_list,"task","s2p_meta","paq_beh",p.tng.activityMeasure];
end
if ops.do_placeCellAnalysis
    cmpr_list = [cmpr_list,"task","s2p_meta","paq_beh",p.pca.activityMeasure,"tng_all","bcon"];
end
if ops.do_tuningAnalysisDff
    cmpr_list = [cmpr_list,"task","s2p_meta","paq_beh",p.tngn.activityMeasure];
end
if ops.do_tuningAnalysisDffS
    cmpr_list = [cmpr_list,"task","s2p_meta","paq_beh",p.tngnn.activityMeasure];
end
...
if ops.do_sequenceWarpAnalysis
    if ops.warp.do_allTrials
        cmpr_list = [cmpr_list,ops.warp.input+"_all"];
    end
    if ops.warp.do_allTrials_stimVersion
        cmpr_list = [cmpr_list,ops.warp.input+"_all_stimVersion"];
    end
    if ops.warp.do_100t
        cmpr_list = [cmpr_list,ops.warp.input+"_100t"];
    end
    if ops.warp.do_100t_stimVersion
        cmpr_list = [cmpr_list,ops.warp.input+"_100t_stimVersion"];
    end
    cmpr_list = [cmpr_list,"s2p_meta"];
end
if ops.do_decodingAnalysis
    cmpr_list = [cmpr_list,"task","paq_beh",p.sqn.activityMeasure,"tng_all","tng_100t"];
end
if ops.do_nemAnalysis
    cmpr_list = [cmpr_list,"task","s2p_meta","paq_beh","cam_beh",p.nem.activityMeasure];
end
...
if ops.do_behConfoundAnalysis
    cmpr_list = [cmpr_list,"task","paq_beh"]; %[cmpr_list,"task","perf","paq_beh","s2p_meta",p.bcon.activityMeasure];
end
if ops.do_trnsAnalysis
    cmpr_list = [cmpr_list,p.trns.activityMeasure];
end
if ops.do_prepostAnalysis
    cmpr_list = [cmpr_list,p.ppa.activityMeasure_pre,p.ppa.activityMeasure_post];
end
...
for i=1:length(temp)
    if strcmp(temp{i},'F_beh') || strcmp(temp{i},'Fneu_beh') || strcmp(temp{i},'dFF_beh') || strcmp(temp{i},'spks_beh') || ...
            strcmp(temp{i},'dFFn_beh') || strcmp(temp{i},'spksn_beh') || strcmp(temp{i},'dFFnn_beh') || strcmp(temp{i},'dFFns_beh') || strcmp(temp{i},'spksnn_beh') || ...
            strcmp(temp{i},'spksn_pre') || strcmp(temp{i},'spksn_post')
...
if exist('task','var') && exist('thor_beh','var')
    if sum(task.var==1)==length(thor_beh.stimSequence)
...
if ops.do_trnsAnalysis
    disp('- [trns] module')
    act = getActivityMeasure(p.trns,act_struct);
    trnsAnalysis(path,act);
end
...
if ops.do_saveTraces
    disp('- [str] module')
    act = getActivityMeasure(p.tng,act_struct);
    saveTraces(info,iscell,ops,p,path,task,s2p_meta,paq_beh,act);
end

if ops.do_placeCellAnalysis
    disp('- [pca] module')
    act = getActivityMeasure(p.tng,act_struct);
    [pca] = placeCellAnalysis(info,iscell,ops,p,path,task,paq_beh,act,tng_all,bcon);
    out.pca = pca;
end

if ops.do_tuningAnalysisDff
    disp('- [tngn] module')
    act = getActivityMeasure(p.tngn,act_struct);
    [tngn,~] = tuningAnalysisDff(info,iscell,ops,p,path,task,s2p_meta,paq_beh,act);
    out.tng = tngn;
end

if ops.do_tuningAnalysisDffS
    disp('- [tngnn] module')
    act = getActivityMeasure(p.tngnn,act_struct);
    [tngnn,~] = tuningAnalysisDffS(info,iscell,ops,p,path,task,s2p_meta,paq_beh,act);
    out.tng = tngnn;
end
...
if ops.do_sequenceWarpAnalysis
    disp('- [warp] module')
    if ops.warp.do_allTrials
        sequenceWarpAnalysis(info,iscell,ops,p,path,eval(ops.warp.input+"_all"),'all');
    end
    if ops.warp.do_allTrials_stimVersion
        sequenceWarpAnalysis(info,iscell,ops,p,path,eval(ops.warp.input+"_all_stimVersion"),'all_stimVersion');
    end
    if ops.warp.do_100t
        warp = sequenceWarpAnalysis(info,iscell,ops,p,path,eval(ops.warp.input+"_100t"),'100t');
    end
    if ops.warp.do_100t_stimVersion
        sequenceWarpAnalysis(info,iscell,ops,p,path,eval(ops.warp.input+"_100t_stimVersion"),'100t_stimVersion');
    end
end
...
if ops.do_decodingAnalysis
    ...
    decodingAnalysis(info,iscell,ops,p,path,task,paq_beh,act,tng_all,tng_100t);
end

if ops.do_nemAnalysis
    disp('- [nem] module')
    act = getActivityMeasure(p.nem,act_struct);
    nemAnalysis(info,iscell,ops,p,path,task,paq_beh,cam_beh,act);
end
if ops.do_nemAnalysis_cmpr
...
if ops.do_imprintingAnalysis
    ...
    [impr] = imprintingAnalysis(info,iscell,ops,p,path,task,trg,paq_beh,act);
    out.impr = impr;
end

if ops.do_prepostAnalysis
    disp('- [ppa] module')
    act_pre = act_struct.spksn_pre;
    act_post = act_struct.spksn_post;
    [ppa] = prepostAnalysis(info,iscell,ops,p,path,act_pre,act_post);
    out.ppa = ppa;
end
...
if ops.do_behConfoundAnalysis
    disp('- [bcon] module')
    % act = getActivityMeasure(p.bcon,act_struct);
    behConfoundAnalysis(info,ops,p,path,task,paq_beh);
end

function [dec,dec_mdl] = decodingCore(dec,this_nft_binned,prop,p)
% this_nft_binned = nft_binned;

%% Create decoder struct

this_data = this_nft_binned(find(prop.iscell==1),dec.bins.bins,dec.cv.trialsForTrainingSet.combined);
decs_train = createDecoderStruct(dec,this_data,dec.cv.trialsForTrainingSet,{'A_H_O','X_H_O','A_CR_O','X_CR_O'});
this_data = this_nft_binned(find(prop.iscell==1),dec.bins.bins,dec.cv.trialsForCompleteSet.combined);
decs_complete = createDecoderStruct(dec,this_data,dec.cv.trialsForCompleteSet,{'AB','XY','AY','XB'});

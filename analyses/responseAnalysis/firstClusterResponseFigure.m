function F = firstClusterResponseFigure(s2p_meta,iscell,trg,resp,p,info)
...
subplot(2,4,1)
clusterResponseMap(s2p_meta,iscell,trg,resp,resp.firstCluster.clusterIdx(1,1),1,1); 
if strcmp(resp.stimType,'seq')
    title('First cluster (A)')
elseif strcmp(resp.stimType,'ctrl')
    title(['Example first cluster (',num2str(resp.firstCluster.clusterIdx(1,1)),')'])
end

subplot(2,4,2)
clusterResponseMap(s2p_meta,iscell,trg,resp,resp.firstCluster.clusterIdx(1,2),1,1); 
if strcmp(resp.stimType,'seq')
    title('First cluster (X)')
elseif strcmp(resp.stimType,'ctrl')
    title(['Example first cluster (',num2str(resp.firstCluster.clusterIdx(1,2)),')'])
end
...
subplot(2,4,3)
hold on
bar(these_labels,[resp.firstCluster.responders_ext.numRespTargeted,resp.firstCluster.responders_0d5z_ext.numRespTargeted,resp.firstCluster.responders_1z_ext.numRespTargeted],'FaceColor',p.col.photostim)
yline(resp.firstCluster.responders_ext.numIdentifiedTargeted,'LineStyle',':','LineWidth',2);
ylim([0,resp.firstCluster.responders_ext.numAllTargeted])
yticks([0:40:240])
...
subplot(2,4,4)
hold on
bar(these_labels,[resp.firstCluster.responders_ext.proportion_RespNonTargeted_RespTargeted,resp.firstCluster.responders_0d5z_ext.proportion_RespNonTargeted_RespTargeted,resp.firstCluster.responders_1z_ext.proportion_RespNonTargeted_RespTargeted],'FaceColor',p.col.darkGray)
yline(0,'LineStyle','-');
...
temp = resp.firstCluster.avgAct_net(:);
hold on
histogram(temp(find(resp.firstCluster.stats_sig_pos(:)==1 & resp.firstCluster.responders_main.targeted(:)==1)),'Normalization','probability','BinWidth',0.25,'FaceColor',p.col.photostim)
histogram(temp(find(resp.firstCluster.stats_sig_pos(:)==1 & resp.firstCluster.responders_main.targeted(:)==0)),'Normalization','probability','BinWidth',0.25,'FaceColor',p.col.darkGray)
...
temp = resp.firstCluster.stats_sig_pos(:);
...
temp = resp.firstCluster.stats_sig_neg(:);
temp2 = discretize(trg.dist_closestLaser(:),[0:binsize:400]);
responseProbability = nan(nanmax(temp2),1);
for j=1:nanmax(temp2)
    responseProbability(j) = nanmean(temp(find(temp2==j)));

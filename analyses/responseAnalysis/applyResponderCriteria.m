...
resp.(fieldname_neg).respAll = nan(size(resp.stats_sig_neg));
for i=1:size(resp.stats_sig_neg,2)
    for j=1:size(resp.stats_sig_neg,1)
        if isnan(amplitudeCriterion)
            resp.(fieldname_neg).respAll(j,i) = resp.stats_sig_neg(j,i);
        elseif ~isnan(resp.stats_sig_pos(j,i))
            resp.(fieldname_neg).respAll(j,i) = resp.stats_sig_neg(j,i) && (resp.avgAct_net(j,i)<amplitudeCriterion);
        end
    end
end
...
%% Get idcs of different cell groups by cluster

resp.(fieldname).idcs_respTargeted = cell(1,size(resp.(fieldname).respTargeted,2));
for i=1:size(resp.(fieldname).respTargeted,2)
    resp.(fieldname).idcs_respTargeted{i} = resp.(fieldname).targetedCells(resp.(fieldname).respTargeted(:,i)==1,i);
end

resp.(fieldname).idcs_targeted = cell(1,size(resp.(fieldname).targetedCells,2));
for i=1:size(resp.(fieldname).targetedCells,2)
    resp.(fieldname).idcs_targeted{i} = rmmissing(resp.(fieldname).targetedCells(:,i));
end

resp.(fieldname).idcs_resp = cell(1,size(resp.(fieldname).respAll,2));
for i=1:size(resp.(fieldname).respAll,2)
    resp.(fieldname).idcs_resp{i} = find(resp.(fieldname).respAll(:,i)==1);
end

...
these_labels = categorical({char('significant')});
these_labels = reordercats(these_labels,{char('significant')});

subplot(2,4,2)
hold on
bar(these_labels,[resp.responders_main.numRespTargeted],'FaceColor',p.col.photostim)
yline(resp.responders_main.numIdentifiedTargeted,'LineStyle',':','LineWidth',2);
ylim([0,resp.responders_main.numAllTargeted])
yticks([0:40:240])
...
subplot(2,4,3)
hold on
bar(these_labels,[resp.responders_main.proportion_RespNonTargeted_RespTargeted],'FaceColor',p.col.darkGray)
yline(0,'LineStyle','-');
...
subplot(2,4,4)
hold on
v=bar(these_labels,[resp.responders_main.specificity_respTargeted,resp.responders_main.specificity_respIpsiGroup-resp.responders_main.specificity_respTargeted,resp.responders_main.specificity_respContraGroup,resp.responders_main.specificity_respNoGroupCells]);
...
hold on
for i=1:1
    fieldname = char('responders_main');

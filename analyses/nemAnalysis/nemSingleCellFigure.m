...
temp = reshape(nemd.full.Ypred(prop.trial_frames_binned,idx)',[],size(prop.trial_frames_binned,1),prop.numTrials);
...
plot(temp,nemd.full.Ypred(this_window,idx),'Color',p.col.photostim,'LineWidth',2)
...
h=barh(diag(nemd.full.coefs(idx,2:end)),'stacked','BaseValue',0,'FaceColor','none','EdgeColor','none');
...
for j=1:nem.numPredictorGroups
    this_predictorGroup = nem.predictorGroups.names{j};
    this_data(j) = nansum(nemd.full.coefs(idx,1+nem.predictorGroups.idcs{j}));
end
